...
class MeshUnpool_F(nn.Module):
    def __init__(self, unroll_target):
        super(MeshUnpool_F, self).__init__()
    ...
    def forward(self, features, meshes):
        batch_size, nf, faces = features.shape
        groups = [self.pad_groups(mesh.get_groups(), faces) for mesh in meshes]
        unroll_mat = torch.cat(groups, dim=0).view(batch_size, faces, -1)

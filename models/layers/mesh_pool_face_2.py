...
class MeshPoolFace2(nn.Module):
    
    def __init__(self, target, multi_thread=False):
        super(MeshPoolFace2, self).__init__()
    ...
    def __pool_main(self, mesh_index):
        ...
        edge_mask = np.ones(mesh.edges_count, dtype=np.uint8)
        face_mask = np.ones(mesh.face_count, dtype=np.uint8)
        ...
        while mesh.face_count > self.__out_target:
            ...
            if face_mask[face_id]:
                ...
                min_n = -1
                for n in neighbors:
                    if face_mask[n] and np.where(np.asarray(queue)[:,1]==n)[0].size==1:
                        val = queue[np.where(np.asarray(queue)[:,1]==n)[0][0]][0]
                        if val<min_val:
                            min_val = val
                            min_n = n
                if min_n!=-1:
    ...
    def __pool_edge(self, mesh, edge_id, edge_mask, face_mask, edge_groups, f1, f2):
        ...
        if self.has_boundaries(mesh, edge_id):
            return False
        elif self.__clean_side(mesh, edge_id, edge_mask, edge_groups, 0)\
            and self.__clean_side(mesh, edge_id, edge_mask, edge_groups, 2) \
            and self.__is_one_ring_valid(mesh, edge_id):

            # Merge side edges
            self.__merge_edges[0] = self.__pool_side(mesh, edge_id, edge_mask, edge_groups, 0)
            self.__merge_edges[1] = self.__pool_side(mesh, edge_id, edge_mask, edge_groups, 2)
            mesh.merge_vertices(edge_id)

            # Remove edge
            edge_mask[edge_id] = False
            MeshPoolFace2.__remove_group(mesh, edge_groups, edge_id)
            mesh.edges_count -= 1
            ...
            #Update neighbors of f1
            neighbors = mesh.gemm_faces[f1]
            neighbors = neighbors[neighbors!=f2]
            mesh.gemm_faces[neighbors[0], np.where(mesh.gemm_faces[neighbors[0]] == f1)[0][0]] = neighbors[1]
            mesh.gemm_faces[neighbors[1], np.where(mesh.gemm_faces[neighbors[1]] == f1)[0][0]] = neighbors[0]

            # Update neighbors of f2
            neighbors = mesh.gemm_faces[f2]
            neighbors = neighbors[neighbors != f1]
            mesh.gemm_faces[neighbors[0], np.where(mesh.gemm_faces[neighbors[0]] == f2)[0][0]] = neighbors[1]
            mesh.gemm_faces[neighbors[1], np.where(mesh.gemm_faces[neighbors[1]] == f2)[0][0]] = neighbors[0]
    ...
    def __clean_side(self, mesh, edge_id, mask, edge_groups, side):
        if mesh.edges_count <= self.__out_target:
            return False
        invalid_edges = MeshPoolFace2.__get_invalids(mesh, edge_id, edge_groups, side)
    ...
    def __pool_side(self, mesh, edge_id, mask, edge_groups, side):
        info = MeshPoolFace2.__get_face_info(mesh, edge_id, side)
        ...
        MeshPoolFace2.__union_groups(mesh, edge_groups, key_b, key_a)
        MeshPoolFace2.__union_groups(mesh, edge_groups, edge_id, key_a)
        mask[key_b] = False
        mesh.edges_in_face[mesh.edges_in_face == key_b] = key_a
        MeshPoolFace2.__remove_group(mesh, edge_groups, key_b)
    ...
    def __get_invalids(mesh, edge_id, edge_groups, side):
        ...
        shared_items = MeshPoolFace2.__get_shared_items(other_keys_a, other_keys_b)
        if len(shared_items) == 0:
            ...
            MeshPoolFace2.__redirect_edges(mesh, edge_id, side, update_key_a, update_side_a)
            MeshPoolFace2.__redirect_edges(mesh, edge_id, side + 1, update_key_b, update_side_b)
            MeshPoolFace2.__redirect_edges(mesh, update_key_a, MeshPoolFace2.__get_other_side(update_side_a), update_key_b, MeshPoolFace2.__get_other_side(update_side_b))
            MeshPoolFace2.__union_groups(mesh, edge_groups, key_a, edge_id)
            MeshPoolFace2.__union_groups(mesh, edge_groups, key_b, edge_id)
            MeshPoolFace2.__union_groups(mesh, edge_groups, key_a, update_key_a)
            MeshPoolFace2.__union_groups(mesh, edge_groups, middle_edge, update_key_a)
            MeshPoolFace2.__union_groups(mesh, edge_groups, key_b, update_key_b)
            MeshPoolFace2.__union_groups(mesh, edge_groups, middle_edge, update_key_b)
    ...
    def __remove_triplete(mesh, mask, edge_groups, invalid_edges):
        ...
        for edge_key in invalid_edges:
            ...
            mask[edge_key] = False
            MeshPoolFace2.__remove_group(mesh, edge_groups, edge_key)
        mesh.edges_count -= 3
        vertex = list(vertex)
        assert(len(vertex) == 1)
